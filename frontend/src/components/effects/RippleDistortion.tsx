...
// Vendored from React Bits (RippleDistortion, JS + CSS variant) and ported to
// TypeScript for this project's tsconfig. Kept close to upstream so future
// updates from the source are easy to diff; only the container background
// (RippleDistortion.css) and this file's types/guard clause are local changes.
...
const RippleDistortion = ({
  ...
  useEffect(() => {
    ...
    // WebGL isn't guaranteed everywhere (headless/sandboxed browsers, old
    // devices). This is a purely decorative layer, so failing to init must
